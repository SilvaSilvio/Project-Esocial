    ...
    try:
        ...
        if file_extension == '.csv':
            ...
            for col in df.columns:
                ...
                # Ajusta valores numéricos do eSocial
                elif 'VALOR' in col.upper() and '_ESOCIAL' in col.upper():
                    df[col] = pd.to_numeric(df[col], errors='coerce')
                    df[col] = df[col].apply(lambda x: x/100 if pd.notnull(x) and x > 100 else x)
    ...
    def convert_br_number(x):
        try:
            if isinstance(x, (int, float)):
                # Se for um valor do eSocial (verificando pelo nome da coluna)
                if '_ESOCIAL' in valor_col.upper() and x > 100:
                    return float(x) / 100
                return float(x)
            if pd.isna(x) or x == '':
                return 0.0
            ...
            if ',' in x and '.' in x:
                ...
                # Apenas números: 1234
                # Se for um valor do eSocial
                if '_ESOCIAL' in valor_col.upper() and len(x) > 2:
                    return float(x) / 100
                return float(x)
        except:
            return None
...
def process_esocial_file(input_file):
    ...
    try:
        ...
        # Encontra o índice da coluna 'CATEGORIA DO TRABALHADOR'
        categoria_idx = None
        for idx, col in enumerate(df_esocial.columns):
            if 'CATEGORIA DO TRABALHADOR' in str(col).upper():
                categoria_idx = idx
                break
        ...
        # Separa as colunas fixas e as colunas a serem transformadas
        colunas_fixas = df_esocial.columns[:categoria_idx + 1].tolist()
        colunas_transformar = df_esocial.columns[categoria_idx + 1:].tolist()
        ...
        colunas_finais = colunas_fixas + ['Código', 'Descrição', 'Descrição Completa', 'Valor']
        df_transformed = df_transformed[colunas_finais]
        
        # Ordena o DataFrame
        df_transformed = df_transformed.sort_values(by=['Código', 'Valor'], na_position='last')
...
def main():
    try:
        ...
        with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
            ...
            # Adiciona a coluna de diferença antes de salvar a planilha Valores por eSocial
            df_valores_esocial = result_lg_depara['valores_por_esocial'].copy()
            df_valores_esocial['VALOR_LIQUIDO'] = pd.to_numeric(df_valores_esocial['VALOR_LIQUIDO'], errors='coerce')
            df_valores_esocial['Valor_eSocial'] = pd.to_numeric(df_valores_esocial['Valor_eSocial'], errors='coerce')
            df_valores_esocial['Diferença'] = df_valores_esocial['VALOR_LIQUIDO'] - df_valores_esocial['Valor_eSocial']
            df_valores_esocial['Diferença'] = df_valores_esocial['Diferença'].fillna(0)
            
            # Salva a planilha Valores por eSocial
            df_valores_esocial.to_excel(writer, sheet_name='Valores por eSocial', index=False)
            
            # Salva a planilha do eSocial
            df_esocial.to_excel(writer, sheet_name='eSocial', index=False)
            
            # Aplica a formatação condicional na planilha Valores por eSocial
            worksheet = writer.sheets['Valores por eSocial']
            cor_igual = PatternFill(start_color='90EE90', end_color='90EE90', fill_type='solid')  # Verde claro
            cor_diferente = PatternFill(start_color='FFB6C1', end_color='FFB6C1', fill_type='solid')  # Vermelho claro
            
            # Encontra o índice da coluna Diferença
            diferenca_col = None
            for idx, col in enumerate(df_valores_esocial.columns):
                if col == 'Diferença':
                    diferenca_col = idx
                    break
            
            if diferenca_col is not None:
                for row_idx, row in enumerate(worksheet.iter_rows(min_row=2), start=2):
                    valor_diferenca = row[diferenca_col].value
                    if valor_diferenca is not None:
                        cor = cor_igual if abs(float(valor_diferenca)) < 0.01 else cor_diferente
                        for cell in row:
                            cell.fill = cor
